...
_BUCKET_URL = "s3://covid19datahub-dataset/covid19/covid19datahub"
...
def get_covid_datahub(start_date: date, end_date: date) -> Iterable[pd.DataFrame]:
    while start_date < end_date:
        logger.debug(f"Downloading data for partition {start_date}...")

        partitions = _get_hive_partition_filter(dt=start_date)
        df = DeltaTable(_BUCKET_URL, storage_options=_STORAGE_OPTIONS).to_pandas(partitions=partitions)
